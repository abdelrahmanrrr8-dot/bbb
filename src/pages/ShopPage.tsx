    ...
    <>
      <div className="mb-5">
        ...
        <div className="space-y-1">
          <button onClick={() => setCategory(undefined)} className={`w-full text-right px-3 py-2 rounded-sm text-sm font-medium transition-colors ${!selectedCategory ? 'bg-gold-400 text-jet' : 'text-silver-700 hover:bg-silver-100'}`}>جميع الأقسام</button>
          {CATEGORIES.map((cat) => (
            <button key={cat.key} onClick={() => setCategory(cat.key)} className={`w-full text-right px-3 py-2 rounded-sm text-sm font-medium transition-colors flex items-center gap-2 ${selectedCategory === cat.key ? 'bg-gold-400 text-jet' : 'text-silver-700 hover:bg-silver-100'}`}>
      ...
      {brands.length > 0 && (
        <div className="mb-5">
          <p className="text-xs font-bold text-silver-600 mb-2 uppercase">العلامة التجارية</p>
          <div className="space-y-1">
            <button onClick={() => setSelectedBrand(null)} className={`w-full text-right px-3 py-2 rounded-sm text-sm font-medium transition-colors ${!selectedBrand ? 'bg-gold-400 text-jet' : 'text-silver-700 hover:bg-silver-100'}`}>الكل</button>
            {brands.map((brand) => (
              <button key={brand} onClick={() => setSelectedBrand(brand)} className={`w-full text-right px-3 py-2 rounded-sm text-sm font-medium transition-colors ${selectedBrand === brand ? 'bg-gold-400 text-jet' : 'text-silver-700 hover:bg-silver-100'}`}>{brand}</button>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="mb-5">
        ...
        {maxPrice !== null && <button onClick={() => setMaxPrice(null)} className="text-xs text-alert hover:underline mt-1">إلغاء فلتر السعر</button>}
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 animate-fade-in">
      <div className="mb-4">
        ...
        {searchQuery && <p className="text-silver-500 mt-1 text-sm">نتائج البحث عن: <span className="font-semibold text-jet">"{searchQuery}"</span></p>}
      </div>

      <div className="flex gap-6">
        <aside className="hidden md:block w-60 flex-shrink-0">
          <div className="bg-white rounded-sm shadow-card p-5 sticky top-44">
            <h3 className="font-bold text-jet mb-4 flex items-center gap-2 text-sm"><SlidersHorizontal className="w-4 h-4 text-gold-500" />تصفية</h3>
            <FilterContent />
          </div>
        </aside>
        ...
        <div className="flex-1">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {sortedAndFiltered.map((product) => <ProductCard key={product.id} product={product} onQuickView={setQuickView} />)}
            </div>
          )}
        </div>
      </div>
        ...
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute top-0 right-0 w-72 max-w-[85%] h-full bg-white p-5 animate-slide-left overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowFilters(false)} className="p-2 rounded-sm hover:bg-silver-100"><X className="w-5 h-5 text-silver-600" /></button>
            </div>
            <FilterContent />
          </div>
        </div>
