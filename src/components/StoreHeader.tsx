...
export function StoreHeader({ onCartClick }: StoreHeaderProps) {
  ...
  useEffect(() => {
    if ((route.path === '/shop' || route.path === '/products') && route.params.q) {
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="flex items-center gap-1 md:gap-2 order-3">
              {[Facebook, Youtube, Twitter, Instagram].map((Icon, i) => (
                <a key={i} href="#" className="p-2 rounded-sm hover:bg-black/10 transition-colors" aria-label="social">
                  <Icon className="w-4 h-4 md:w-5 md:h-5 text-jet" />
                </a>
              ))}
          ...
          <div className="max-w-3xl mx-auto p-4">
            <form onSubmit={handleSearch}>
              <div className="relative">
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="ابحث عن منتج..." autoFocus
      ...
      <nav className="bg-jet hidden md:block">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-center">
            {navItems.map((item) => (
              <button key={item.label} onClick={() => navigate(item.path)} className={isActive(item.path) ? 'nav-link-active' : 'nav-link'}>
                {item.label}
              </button>
            ))}
          </div>
      ...
      {mobileMenuOpen && (
        <nav className="md:hidden bg-jet animate-slide-up">
          <div className="px-4 py-2">
            {navItems.map((item) => (
              <button key={item.label} onClick={() => { navigate(item.path); setMobileMenuOpen(false); }}
                className={`block w-full text-right py-3 text-sm font-semibold border-b border-jet-50 ${isActive(item.path) ? 'text-gold-400' : 'text-silver-200'}`}>
                {item.label}
              </button>
            ))}
          </div>
