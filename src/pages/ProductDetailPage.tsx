...
import { ShoppingCart, Minus, Plus, ArrowRight, Check, Truck, ShieldCheck, RotateCcw, Star } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid md:grid-cols-2 gap-8 animate-pulse">
          ...
          <div className="space-y-4">
            <div className="h-8 bg-silver-100 rounded w-3/4" />
            <div className="h-6 bg-silver-100 rounded w-1/4" />
            <div className="h-24 bg-silver-100 rounded" />
          </div>
        </div>
      </div>
    );
  ...
  if (!product) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        ...
        <button onClick={() => navigate('/products')} className="btn-gold mt-4">العودة للمنتجات</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 animate-fade-in">
      ...
      <nav className="flex items-center gap-2 text-sm text-silver-500 mb-6">
        ...
        <button onClick={() => navigate(`/products?category=${product.category}`)} className="hover:text-gold-500 transition-colors">{product.category}</button>
        <ArrowRight className="w-3.5 h-3.5" />
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-sm shadow-card overflow-hidden relative">
          {product.image_url ? (
            ...
            <div className="w-full aspect-square bg-silver-100 flex items-center justify-center">
              <ShoppingCart className="w-24 h-24 text-silver-400" />
            </div>
          )}
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-baseline gap-3 mb-4">
            ...
            {sale && product.old_price && (
              <span className="text-lg line-through text-silver-500">{formatPrice(product.old_price)}</span>
            )}
          </div>

          {product.description && (
            <p className="text-silver-600 leading-relaxed mb-6">{product.description}</p>
          )}

          {/* Stock */}
          <div className="mb-6">
            {product.stock > 0 ? (
              <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-sm bg-green-50 text-green-700 text-sm font-medium">
                <Check className="w-4 h-4" /> متوفر في المخزون ({product.stock} قطعة)
              </span>
            ) : (
              <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-sm bg-red-50 text-alert text-sm font-medium">
                نفذت الكمية
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="flex items-center border border-silver-300 rounded-sm overflow-hidden">
              <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-11 h-11 flex items-center justify-center hover:bg-silver-100 transition-colors">
                <Minus className="w-4 h-4 text-jet" />
              </button>
              <span className="w-12 text-center font-bold text-jet">{quantity}</span>
              <button onClick={() => setQuantity(Math.min(product.stock, quantity + 1))} className="w-11 h-11 flex items-center justify-center hover:bg-silver-100 transition-colors">
                <Plus className="w-4 h-4 text-jet" />
              </button>
            </div>
